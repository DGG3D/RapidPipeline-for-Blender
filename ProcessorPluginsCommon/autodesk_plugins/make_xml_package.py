...
def add_RuntimeRequirements(parent:ET.Element, os:str, platform:str, series_min:str, series_max:str) -> ET.Element:
    if series_max:
        ...
        ET.SubElement(parent, "RuntimeRequirements", OS=os, Platform=platform, SeriesMin=series_min)
...
def make_XML_package(product: str, version: str, output_folder:str):
    ...
    if product == "Maya":
        add_RuntimeRequirements(package, "win64", product, "2025", "")
        add_mainScript(package, "win64", product, "2025", "")
    else:
...
def copy_rpde_files(rpde_folder:str, output_folder:str):
    ...
    hoops_dir = rpde_folder / Path("hoops")
...
def package_folder(hoops_build, input_folder:str, archive_path:str, versionNote:str):
    package_folder = Path(input_folder)

    addedNotes = ""
    if versionNote:
        addedNotes += "_" + versionNote
    if hoops_build:
        addedNotes += "_CAD"

    if addedNotes:
        outPath = Path(archive_path)
        new_stem = outPath.stem + addedNotes
...
if __name__ == "__main__":
    ...
    parser.add_argument("-n", "--note", type=str, help="Version note, e.g. platform.")
    ...
    output_folder = args.output + f"/{package_name[product]}"
    output_folder_plugin = output_folder
    if args.product == "maya":
        output_folder_plugin += "/Contents/plugin"

    copy_input_files(args.input, args.rpde, args.schema, output_folder_plugin, args.version)
    hoops_build = copy_rpde_files(args.rpde, output_folder_plugin)
