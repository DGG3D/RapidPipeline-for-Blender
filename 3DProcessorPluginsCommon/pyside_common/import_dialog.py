...
class ImportDialog(QtWidgets.QDialog):
    def __init__(self, parent: QtWidgets.QWidget, schema:dict, actions:List[action_utils.MagicAction], show_preview:bool):
        ...
        self.actions_container.setFixedWidth(350)
...
class ExportDialog(ImportDialog):
    def __init__(self, parent: QtWidgets.QWidget, schema:dict, actions:List[action_utils.MagicAction], file_format:str, show_preview:bool):
        filtered_actions = [a for a in actions if a.file_format == file_format]
        print(f"Export Action for {file_format}: {filtered_actions}")
        super().__init__(parent, schema, [filtered_actions[0]], show_preview) # we only want one action
